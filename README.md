...
###### 1) Download variables from the file exclude_variables.conf (single point of entry for multiple applications) to exclude from the analysis and blocking:
...
```
###### 2) Create a Chain ddos-deflate from iptables and forward to it all input traffic.
